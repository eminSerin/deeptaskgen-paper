...
REF_MODEL = torch.load(
    op.join(
        ABS_PATH, "experiments/training/results/unetminimal_100_0.001/best_corr.ckpt"
    ),
    map_location="cpu",
)
...
CHECKPOINT_MAP = {
    "emotion-faces-shapes": op.realpath(
        op.join(
            ABS_PATH,
            "experiments/transfer_learning/hcp_development/results/finetuned_50_0.001_emotion-faces-shapes/best_corr.ckpt",
        )
    ),
    "gambling-reward": op.realpath(
        op.join(
            ABS_PATH,
            "experiments/transfer_learning/hcp_development/results/finetuned_50_0.001_gambling-reward/best_corr.ckpt",
        )
    ),
}
...
for cont in CHECKPOINT_MAP:
    ...
    trainer.save_checkpoint(
        CHECKPOINT_MAP[cont].replace("best_corr.ckpt", "best_corr_all.ckpt")
    )
