...
MASK = op.join(ABS_PATH, "utils/templates/MNI_2mm_GM_mask_crop.nii")
...
if __name__ == "__main__":
    for task in TASK_MAP:
        ...
        for pred_task in TASK_MAP[task]["pred"]:
            ...
            pred_cont = pred_cont[:, :, TASK_MAP[task]["idx"], :]
            ...
            np.save(
                op.join(
                    op.dirname(TASK_MAP[task]["pred"][pred_task]),
                    f"corr_scores_{task}.npy",
                ),
                corr,
            )
            print(f"Computing Dice AUC for {task} - {pred_task}...")
            dice = dice_auc(
                pred_cont.reshape(N_SUBJ, -1),
                test_contrasts.reshape(N_SUBJ, -1),
                n_jobs=N_JOBS,
